...
Packet DistanceVector::createDVPacket(unsigned short neighborID)
{
    ...
    for (const auto &nbr : forwardingTable.table)
    { // added const and & because we won't be modifying in the for loop, so more efficient (won't make a copy)
        router_id routeDestID = nbr.first;
        router_id nextHop = nbr.second.nextHop;
        cost routeCost = nbr.second.routeCost;

        if (nextHop == neighborID && routeDestID != nextHop)
        { // Poison Reverse: If a node (C) goes through its neighbor (B) to get to a route destination (A), then have C tell B its distance to A is infinite
            routeCost = USHRT_MAX;
        }
...
for (auto it = bigTable.table.begin(); it != bigTable.table.end(); ) {
    router_id destID = it->first;
    ...
    if (bigTable.table[destID].count(neighborID) > 0) {
        if (dvPayload.table.count(destID) == 0) {
            ...
            // Get the best alternative route
            RouteInfo bestRoute = bigTable.getBestRoute(destID);

            if (bestRoute.routeCost != USHRT_MAX - 1) {
                // If a next best route exists, update the forwarding table
                forwardingTable.updateRoute(destID, bestRoute.nextHop, bestRoute.routeCost, verbose);
            } else {
                // If no next best route exists, remove the destination from the forwarding table
                if (verbose) {
            ...
            // Skip increment since we already moved `it` to the next element
            continue;
        }
    }

    // Safely move to the next element if no erasure occurred
    ++it;
}
...
void DistanceVector::handleCostChange(port_num port, int changeCost)
{
    ...
    for (auto &pair : bigTable.table)
    {
        ...
        if (destination == neighborID)
        { // skip because we added changecost to this path immediately
            continue;
        }
